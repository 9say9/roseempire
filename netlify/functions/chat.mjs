import prompts from "../../chat-prompts.json" with { type: "json" };
...
export default async (request) => {
  ...
  if (!apiKey) {
    return jsonResponse(
      {
        error:
          "GEMINI_API_KEY is not set in Netlify. Site settings → Environment variables → add GEMINI_API_KEY, then redeploy.",
      },
      503
    );
  }
